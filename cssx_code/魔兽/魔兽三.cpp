...
protected:
	bool shotDead = 0;
	bool reachEnemyHead = 0;        //sort:dragon=1, ninja=2, iceman=3, lion=4, wolf=5
...
public:
	...
	int make(int sort) {      //制造sort型worrior，返回按sq下一个该制造的sort编号
		...
		if (sort == 1) {      // Dragon made!
			...
			pWarrior[tot_warrior] = pD;           //把临时pD放进抽象类指针数组
		}
		...
		if (sort == 4) {      // Lion made!
			...
			pWarrior[tot_warrior] = pL;            //把临时pL放进抽象类指针数组
		}
		if (sort == 5) {      // Wolf made!
			CWolf *pW = new CWolf(this);           //生成一个新的Dragon，把它的地址放在*pD里
			pW->id = tot_warrior;
			pWarrior[tot_warrior] = pW;            //把临时pL放进抽象类指针数组
		}
		return sq[sort];
	}
	void allocate() {
...
CWarrior::~CWarrior() {                             //warrior死亡！（虚函数）
	...
	pLocOfWarrior->pWarriorOfCity[pHeadOfWarrior->name] = NULL;
}
...
int main() {
	...
	cin >> n; all = n;
	while (n--) {
		someoneWin = 0; gameOver = 0;
		t = 0;     //注意：每次游戏前时间归零！
		cout << "Case " << all - n << ":" << endl;
		int sqi = 3, sqj = 4;    //i和j记录head下一个该制造哪种warrior
		cin >> M >> N >> R >> K >> T;
		...
		for (int i = 0; i <= N + 1; i++) pCity[i] = new CCity(i);
		int hour = 0, min = 0;
...
0.每个case前清零全局变量！
1.僵尸不能扔炸弹，但可以射箭！
2.僵尸如果在40未清除，要在55前手动清理之
3.Weapon易主要改pMaster
4.一击致命就不会有反击，这要反映在IwillDieHere里边
5.City易帜两边的changeFlag要分别处理
6.Sword忘记wearOut(holdWeapon时也有可能wearOut！)
7.输出错误
8.重复award战胜warrior
9.武士降生时，司令部没更新pWarriorOfCity
10.delete之后原值同this的指针会指向垃圾内存，要手动设为NULL！
11.55之后时间t要增加到00
*/