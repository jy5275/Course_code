...
void PrintTime() {            //时间输出函数！
	if (t < 10) fprintf(dest, "00%d ", t); 
	else if (t < 100) fprintf(dest, "0%d ", t);
	else fprintf(dest, "%d ", t);
}
...
public:
	...
	void make(warrior w) {     //制造sort型warrior
		...
		fprintf(dest, "%s %s %d ", headName(h), sortName(w), pWarrior[tot_wa]->id);
		fprintf(dest, "born with strength %d,", CWarrior::ini_s[w]);
		fprintf(dest, "%d %s in %s headquarter\n", cnt_wa[w], sortName(w), headName(h));
		nowsq = sq[nowsq];

		if (w == dragon) {      // Dragon made!
			pWarrior[tot_wa]->holdWeapon((weapon)(tot_wa % 3));		//赋予Dragon武器
			fprintf(dest, "It has a %s", weaponName((weapon)(tot_wa % 3)));
			pWarrior[tot_wa]->morale = (double)tot_s / (double)CWarrior::ini_s[w]; //士气
			fprintf(dest, ",and it's morale is %.2lf\n", pWarrior[tot_wa]->morale);
		}
...
bool manu(CHeadquarter &head) {        //Head操作函数！
	...
	fprintf(dest, "%s headquarter stops making warriors\n", headName(head.h));
	return 0;
}
...
int main() {
	int n, all;
	fscanf(src, "%d", &n); all = n;
	while (n--) {
		t = 0;     //注意：每次游戏前时间归零
		fprintf(dest, "Case:%d\n", all - n);
		int M; fscanf(src, "%d", &M);
